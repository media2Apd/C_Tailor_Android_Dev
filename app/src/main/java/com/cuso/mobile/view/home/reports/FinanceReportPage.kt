    ...
    Scaffold(
        ...
        Column(
            ...
            AccordionSection(
                ...
                ReportListItem("Profit & Loss Report", "View income, expenses, and net profit for the selected period",
                    initialFavorite = true,
                    onClick = { onReportClick("reports_finance_profit_and_loss_report") })
                ReportListItem("Balance Sheet", "View assets, liabilities, and equity at a specific date.",
                    initialFavorite = true,
                    onClick = { onReportClick("reports_finance_balance_sheet") })
                ReportListItem("Cash Flow Report", "Track cash inflows, outflows, and overall cash position.",
                    initialFavorite = false,
                    onClick = { onReportClick("reports_finance_cash_flow") })
                ReportListItem("Trial Balance", "Verify debit and credit balances before financial statements.",
                    initialFavorite = false,
            ...
            AccordionSection(
                ...
                ReportListItem("Accounts Receivable Report", "Monitor outstanding customer payments.",
                    initialFavorite = true,
                    onClick = { onReportClick("reports_finance_accounts_receivable") })
                ReportListItem("Accounts Payable Report", "Track pending supplier payments.",
                    initialFavorite = false,
                    onClick = { onReportClick("reports_finance_accounts_payable") })
                ReportListItem("Customer Outstanding Report", "View unpaid customer balances.",
                    initialFavorite = false,
                    onClick = { onReportClick("reports_finance_customer_outstanding") })
                ReportListItem("Supplier Outstanding Report", "View unpaid supplier balances.",
                    initialFavorite = false,
            ...
            AccordionSection(
                ...
                ReportListItem("Sales Invoice Report", "Review sales invoices and payment status.",
                    initialFavorite = true,
                    onClick = { onReportClick("reports_finance_sales_invoice") })
                ReportListItem("Purchase Invoice Report", "Review purchase invoices and supplier details.",
                    initialFavorite = false,
                    onClick = { onReportClick("reports_finance_purchase_invoice") })
                ReportListItem("Expense Report", "Monitor business expenses by category",
                    initialFavorite = false,
                    onClick = { onReportClick("reports_finance_expense") })
                ReportListItem("Journal Entry Report", "Review accounting journal entries.",
                    initialFavorite = false,
            ...
            AccordionSection(
                ...
                ReportListItem("Payment Report", "View all incoming and outgoing payments",
                    initialFavorite = true,
                    onClick = { onReportClick("reports_finance_payment") })
                ReportListItem("Payment Status Report", "Monitor paid, pending, and failed payments.",
                    initialFavorite = false,
            ...
            AccordionSection(
                ...
                ReportListItem("Revenue Report", "Analyze revenue across selected periods.",
                    initialFavorite = true,
                    onClick = { onReportClick("reports_finance_revenue") })
                ReportListItem("Expense Analysis Report", "Compare expenses by category.",
                    initialFavorite = false,
                    onClick = { onReportClick("reports_finance_expense_analysis") })
                ReportListItem("Tax Summary Report", "Review collected and payable taxes.",
                    initialFavorite = false,
